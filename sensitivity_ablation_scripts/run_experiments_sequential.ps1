...
$summaryLog = "$logsDir\summary_$timestamp.log"

# Initialize summary log
"=" * 80 | Out-File -FilePath $summaryLog -Encoding utf8
"StaR Sequential Training Summary" | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
$commands = @(
    @{
        Name = "linear"
        Command = "uv run python main_tgn_original.py --dataset_name linear"
        LogFile = "$logsDir\exp1_linear_$timestamp.log"
    },
    @{
        Name = "random_connection_0.65"
        Command = "uv run python main_tgn_original.py --dataset_name random_connection --base_connection_prob 0.65 --preprocessing_data 1"
        LogFile = "$logsDir\exp2_random_0.65_$timestamp.log"
    },
    @{
        Name = "nonlinear"
        Command = "uv run python main_tgn_original.py --dataset_name nonlinear"
        LogFile = "$logsDir\exp3_nonlinear_$timestamp.log"
    },
    @{
        Name = "random_connection_0.85"
        Command = "uv run python main_tgn_original.py --dataset_name random_connection --base_connection_prob 0.85 --preprocessing_data 1"
        LogFile = "$logsDir\exp4_random_0.85_$timestamp.log"
    },
    @{
        Name = "random_connection_0.55"
        Command = "uv run python main_tgn_original.py --dataset_name random_connection --base_connection_prob 0.55 --preprocessing_data 1"
        LogFile = "$logsDir\exp5_random_0.55_$timestamp.log"
    },
    @{
        Name = "random_connection_0.75"
        Command = "uv run python main_tgn_original.py --dataset_name random_connection --base_connection_prob 0.75 --preprocessing_data 1"
        LogFile = "$logsDir\exp6_random_0.75_$timestamp.log"
    },
    @{
        Name = "random_connection_0.45"
        Command = "uv run python main_tgn_original.py --dataset_name random_connection --base_connection_prob 0.45 --preprocessing_data 1"
        LogFile = "$logsDir\exp2_random_0.45_$timestamp.log"
    }
)
