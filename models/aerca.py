...
class AERCA(nn.Module):
    def __init__(self, num_vars: int, hidden_layer_size: int, num_hidden_layers: int, device: torch.device,
        ...
        self.model_name = f'AERCA_{data_name}_{param_hash}'
    ...
    def encoding(self, xs, adjacency_mask=None):
        ...
        if adjacency_mask is not None:
            # adjacency_mask has shape (T - window_size, num_vars, num_vars)
            # winds has shape (T - window_size, window_size, num_vars)
            # We need mask shape to match winds.shape[0]
    ...
    def decoding(self, us, winds, add_u=True, adjacency_mask=None):
        ...
        prev_mask = None
        if adjacency_mask is not None:
            # u_winds has shape (num_u_windows, window_size, num_vars)
            # winds has shape (num_prev_windows, window_size, num_vars)
            # Adjust masks to match respective shapes
            if adjacency_mask.shape[0] >= u_winds.shape[0]:
                # Skip first window_size masks to align with decoding phase
    ...
    def _compute_root_cause_metrics(self, xs, labels, connection_history_list=None, 
            ...
            for i in range(len(xs)):
                ...
                if connection_history_list is not None and len(connection_history_list) > i:
                    ...
                    if conn_history is not None and len(conn_history) > 0:
                        ...
                        # x has shape (T, num_vars), windows will have shape (T-window_size, window_size+1, num_vars)
                        T = x.shape[0]
                        num_windows = T - self.window_size
                        
                        # For each window starting at t, use connection_history at position t+window_size-1
                        # (since connection_history[t] represents connections at timestep t+1)
                        mask_list = []
                        for w in range(num_windows):
                            # Window w starts at position w, ends at w+window_size
                            # Use the connection history at the end of the window
                        ...
                        if mask_list:
                            ...
                            # Expand to match batch dimension: (num_windows, num_vars, num_vars)
        ...
        if connection_history_list is not None and timing_info_list is not None:
            ...
            try:
                ...
                # This is critical: TDS should measure if model maintains scores without edge hints
                ...
                if len(connection_history_list.shape) == 4:
                    ...
                    # Handle case where we need to add a timestep dimension
                    ...
                    # Fill with available connection history
                ...
                # TDS - Measures score stability when edges disappear
                try:
                    # Ensure same timestep count between scores and graphs
                    ...
                    # labels_array has shape (n_samples, n_timesteps, num_vars)
                    # For TDS, we need (n_samples, num_vars) indicating which vars are root causes
                    if labels_array.ndim == 3:
                        # Shape: (n_samples, n_timesteps, num_vars) -> (n_samples, num_vars)
                        labels_array = (labels_array.sum(axis=1) > 0).astype(float)
                    elif labels_array.ndim == 2:
                        # Check if per-node or per-timestep
                        if labels_array.shape[1] == scores_for_tds.shape[2]:
                            # Already per-node
                            pass
                        else:
                            # Need conversion but can't do it, skip
    ...
    def _testing_causal_discover(self, xs, causal_struct_value, connection_history_list=None, mode='static'):
        ...
        if mode == 'dynamic' and connection_history_list is not None:
            self._log_and_print('\n' + '='*80)
            self._log_and_print('DYNAMIC GRAPH CAUSAL DISCOVERY EVALUATION')
        ...
        self._log_and_print('\n' + '='*80)
        self._log_and_print('STATIC GRAPH CAUSAL DISCOVERY EVALUATION')
        self._log_and_print('='*80)
    ...
    def _testing_dynamic_causal_discover(self, xs, connection_history_list):
        ...
        This method evaluates the model's ability to discover causal relationships
        in dynamic graphs where connections change over time. It computes metrics
        at each timestamp and aggregates them following the approach from:
        ...
        self._log_and_print('Testing dynamic causal discovery with time-varying ground truth...')
        self._log_and_print(f'Number of samples: {len(xs)}')
        
        # For each sample, extract per-timestep predictions and ground truth
        all_results = []
            ...
            for sample_idx, (x, connection_history) in enumerate(zip(xs, connection_history_list)):
                ...
                if sample_idx % 10 == 0:
                    ...
                    if sample_idx > 0:
                        ...
                        self._log_and_print(f'  Processing sample {sample_idx}/{len(xs)}...')
                # connection_history shape: (T-1, num_vars, num_vars)
                # Each entry is the adjacency matrix at that timestep
                
                # Get encoder coefficients for this sample
                _, _, _, encoder_coeffs, _, _, _, _ = self._testing_step(x)
                # encoder_coeffs shape: (num_windows, window_size, num_vars, num_vars)
                
                # Extract per-window predictions (median across window, max across lags)
                encoder_estimate_per_window = torch.median(torch.abs(encoder_coeffs), dim=1)[0]  # (num_windows, num_vars, num_vars)
                encoder_estimate_per_window = torch.max(encoder_estimate_per_window, dim=0)[0].cpu().numpy()  # (num_vars, num_vars)
                
                # For dynamic evaluation, we need predictions at each timestep
                # Since we have num_windows predictions, and T-1 ground truth graphs,
                # we need to align them. Use the encoder coefficients across windows.
                
                # Extract per-timestep predictions by taking each window's estimate
                num_windows = encoder_coeffs.shape[0]
                pred_graphs_per_timestep = []
                
                for window_idx in range(num_windows):
                    # For each window, aggregate across lag dimension
                    window_pred = torch.median(torch.abs(encoder_coeffs[window_idx]), dim=0)[0]  # (num_vars, num_vars)
                    pred_graphs_per_timestep.append(window_pred.cpu().numpy())
                
                # Align predictions with ground truth
                # connection_history has T-1 timesteps, predictions may have different length
                # Take the overlapping part
                ...
                # Evaluate dynamic causal discovery for this sample
                try:
        ...
        # Report final aggregated metrics
        total_time = time.time() - start_time
        self._log_and_print('\n' + '='*80)
        self._log_and_print('DYNAMIC CAUSAL DISCOVERY RESULTS (Aggregated across all samples)')
        self._log_and_print('='*80)
        ...
        self._log_and_print('These metrics account for time-varying causal structure')
