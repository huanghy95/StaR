...
class SENNGC(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, adjacency_mask: torch.Tensor = None):
        """
        Args:
            inputs: Input tensor of shape (batch_size, order, num_vars)
            adjacency_mask: Optional adjacency mask of shape (batch_size, num_vars, num_vars) 
                          to mask coefficients. If provided, coefficients are masked element-wise.
        """
        ...
        for k in range(self.order):
            ...
            # Apply adjacency mask if provided (mask out connections that don't exist)
            if adjacency_mask is not None:
                # Expand mask to match coefficient shape: (batch_size, num_vars, num_vars)
                # Mask should be applied element-wise to zero out non-existent connections
