# Time Dimension Ablation Study Script for StaR Models
# This script tests different time encoding dimension configurations
...
$summaryLog = "$logsDir\time_dim_summary_$timestamp.log"

# Initialize summary log
"=" * 80 | Out-File -FilePath $summaryLog -Encoding utf8
"StaR Time Dimension Ablation Study" | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
# Define time dimensions to test
$timeDims = @(8, 16, 24, 48)
...
foreach ($dataset in $datasets) {
    foreach ($timeDim in $timeDims) {
        $commands += @{
            Name = "$($dataset.Name)_time$timeDim"
            Command = "uv run python main_tgn_original.py $($dataset.Command) --time_dim $timeDim"
            LogFile = "$logsDir\time_ablation_$($dataset.Name)_dim${timeDim}_$timestamp.log"
...
for ($i = 0; $i -lt $commands.Count; $i++) {
    ...
    Write-Host "Time Dimension: $($cmd.TimeDim)" -ForegroundColor Yellow
    ...
    "Time Dimension: $($cmd.TimeDim)" | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
Write-Host "TIME DIMENSION ABLATION STUDY COMPLETED" -ForegroundColor Cyan
Write-Host "$('=' * 80)" -ForegroundColor Cyan

"`n`n" + "=" * 80 | Out-File -FilePath $summaryLog -Append -Encoding utf8
"FINAL SUMMARY - TIME DIMENSION ABLATION" | Out-File -FilePath $summaryLog -Append -Encoding utf8
"=" * 80 | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
Write-Host "Time dimensions: $($timeDims -join ', ')" -ForegroundColor Magenta
