# Memory Dimension Ablation Study Script for StaR Models
# This script tests different memory dimension configurations
...
$summaryLog = "$logsDir\memory_dim_summary_$timestamp.log"

# Initialize summary log
"=" * 80 | Out-File -FilePath $summaryLog -Encoding utf8
"StaR Memory Dimension Ablation Study" | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
# Define memory dimensions to test
$memoryDims = @(8, 16, 24)
...
foreach ($dataset in $datasets) {
    foreach ($memDim in $memoryDims) {
        $commands += @{
            Name = "$($dataset.Name)_mem$memDim"
            Command = "uv run python main_tgn_original.py $($dataset.Command) --memory_dim $memDim"
            LogFile = "$logsDir\mem_ablation_$($dataset.Name)_dim${memDim}_$timestamp.log"
...
for ($i = 0; $i -lt $commands.Count; $i++) {
    ...
    Write-Host "Memory Dimension: $($cmd.MemoryDim)" -ForegroundColor Yellow
    ...
    "Memory Dimension: $($cmd.MemoryDim)" | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
Write-Host "MEMORY DIMENSION ABLATION STUDY COMPLETED" -ForegroundColor Cyan
Write-Host "$('=' * 80)" -ForegroundColor Cyan

"`n`n" + "=" * 80 | Out-File -FilePath $summaryLog -Append -Encoding utf8
"FINAL SUMMARY - MEMORY DIMENSION ABLATION" | Out-File -FilePath $summaryLog -Append -Encoding utf8
"=" * 80 | Out-File -FilePath $summaryLog -Append -Encoding utf8
...
Write-Host "Memory dimensions: $($memoryDims -join ', ')" -ForegroundColor Magenta
